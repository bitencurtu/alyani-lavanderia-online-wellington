  ...
  return (
    <AnimatedPage>
      ...
      <FilterBar value={filters} onChange={(p) => setFilters((f) => ({ ...f, ...p }))} showNumero
        onClear={() => setFilters({ dataInicio: undefined, dataFim: undefined, prestadoraId: undefined, numero: undefined })}>
        <div>
            ...
            {rolls.map((r: any, index: number) => (
              <tr key={r.id} className="border-t hover:bg-muted/30" style={{ animationDelay: `${index * 30}ms` }}>
                ...
                <td className="px-1 py-1 text-right whitespace-nowrap">
                  <Button asChild variant="ghost" size="icon"><Link to="/operacao/roll-prestadora/$id" params={{ id: r.id }}><Pencil className="h-4 w-4" /></Link></Button>
        ...
        <DialogContent className="max-w-3xl">
          ...
          <form onSubmit={(e) => { e.preventDefault(); create.mutate(); }}>
            ...
            <div className="rounded-md border bg-card overflow-hidden mb-4">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  ...
                  {novoItens.length === 0 && (
                    <tr><td colSpan={3} className="px-4 py-8 text-center text-muted-foreground">Nenhum item adicionado</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={!novo.prestadora_id || !novo.numero || create.isPending}>Criar Roll</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
